    ...
    GenericTableDelegate.fetchProperties = function (oTable) {
        ...
        return oMetaModel.requestObject(`/${sCollectionPath}/$Type`)
            ...
            .catch(function (oError) {
                ...
                // Fallback properties for SAPIdStatuses
                const mFallbackProperties = {
                    "SAPIdStatuses": [
                        { name: "id", path: "id", label: "ID", dataType: "Edm.Int32", sortable: true, filterable: true, groupable: true },
                    ...
                    const oField = new Field({
                        ...
                        editMode: {
                            ...
                            formatter: function (sPath) {
                                ...
                                console.log(`[SAPID-DELEGATE] Checking edit mode for row: ${rowPath}`);
                                console.log(`[SAPID-DELEGATE] Editing paths: ${sPath}`);
                                ...
                                if (sPath && sPath.includes(",")) {
                                    ...
                                    console.log(`[SAPID-DELEGATE] Split paths:`, aEditingPaths);
                                    const bIsEditable = aEditingPaths.includes(rowPath);
                                    console.log(`[SAPID-DELEGATE] Is row editable: ${bIsEditable}`);
                                    return bIsEditable ? "Editable" : "Display";
                                } else {
                                    // Single row editing: exact match
                                    const bIsEditable = sPath === rowPath;
                                    console.log(`[SAPID-DELEGATE] Single row - Is editable: ${bIsEditable}`);
                                    return bIsEditable ? "Editable" : "Display";
                                }
                            }
                        }
